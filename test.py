...
def test_shipping(f):
    ...
    if not type(costs) is str:
        ...
        return
    if costs != "$1.04":
        print("calculate_shipping_cost((0, 0), (1, 1)) returned {}. Expected result is {}".format(
            costs, "$1.04"))
        return
...
def test_driver(f):
    ...
    if type(driver) is not Driver:
        ...
        return
    if price != 1600:
        print("calculate_driver_cost() did not provide correct final price (expected {}, received {})".format(
            price, 1600))
        return
